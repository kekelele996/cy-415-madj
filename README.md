...
- 首页瀑布流浏览、分类筛选、关键词搜索。
- 物品与个人资料均可设置最多 3 个「想换分类」，首页顶部单独展示双向匹配结果（共同分类数从多到少、发布时间新的在前），匹配区物品不重复进入普通列表；任一方修改分类后首页实时同步。
- 物品详情、物主资料、选择自己的物品发起交换；物主可在详情页直接更新想换分类。
...
- 首次启动会写入演示用户、物品和交换请求。
- 旧数据兼容：`itemApi` / `userApi` 读取时会为缺少 `desired_categories` 字段的历史数据补空数组，旧数据照常浏览。
